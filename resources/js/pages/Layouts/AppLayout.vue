...
<template>
    <header class="px-4 mx-auto sm:px-6 relative pt-4 pb-4 sm:pb-4 border-b border-gray-200 shadow-2xs">
        <nav class="relative flex items-center justify-between sm:h-10 md:justify-center" aria-label="Global">
            <div class="flex items-center flex-1 md:absolute md:inset-y-0 md:left-0">
                <div class="flex items-center justify-between w-full md:w-auto">
                    <a href="/">
                        ...
                        <img class="w-auto h-8 sm:h-10" src="https://www.svgrepo.com/show/470685/space-station.svg" loading="lazy" width="202" height="40">
                    </a>
            ...
            <div class="hidden md:flex md:space-x-10 list-none">
                <Link :href="home()" view-transition
                        class="text-base font-normal text-gray-500 list-none hover:text-gray-900"
                        :class="{ 'text-gray-700 font-semibold': $page.url === '/' }">
                    Mir
                </Link>
                <Link :href="blog()" view-transition
                        class="text-base font-normal text-gray-500 list-none hover:text-gray-900"
                        :class="{ 'text-gray-700 font-semibold': $page.url.startsWith('/blog') }">
                    Blog
                </Link>
                <Link :href="about()" view-transition
                        class="text-base font-normal text-gray-500 list-none hover:text-gray-900"
                        :class="{ 'text-gray-700 font-semibold': $page.url.startsWith('/about') }">
                    About
                </Link>
            </div>
            <div class="hidden md:absolute md:flex md:items-center md:justify-end md:inset-y-0 md:right-0">
                <Link v-if="!user" :href="login()" view-transition
                ...
                <Link v-if="!user" :href="register()" view-transition
                        class="shadow mx-2 inline-flex items-center px-4 py-2 text-base text-gray-900 bg-white border border-transparent rounded-full cursor-pointer font-base hover:bg-gray-50 "
                        :class="{ 'text-gray-700 font-semibold': $page.url.startsWith('/register') }">
                    Register
                </Link>
                <Link v-if="$page.props.can?.accessDashboard" :href="dashboard()" view-transition
                        class="shadow mx-2 inline-flex items-center px-4 py-2 text-base text-gray-900 bg-white border border-transparent rounded-full cursor-pointer font-base hover:bg-gray-50 "
                        :class="{ 'text-gray-700 font-semibold': $page.url.startsWith('/admin') }">
                    Admin
                </Link>
                <Link v-if="user" :href="logout()" method="post" as="button" view-transition
    ...
    <footer class="text-center p-4 bg-gray-900 text-white">
        <a href="#" class="flex items-center justify-center mb-5 text-2xl font-semibold text-gray-500 hover:text-gray-400">
            <img src="https://www.svgrepo.com/show/470685/space-station.svg" class="h-12 mr-3 sm:h-9" alt="Mir Logo">
            Mir
        </a>
        <ul class="flex justify-center mt-5 space-x-5">
            <li><a href="#" class="text-gray-500 hover:text-gray-400">
